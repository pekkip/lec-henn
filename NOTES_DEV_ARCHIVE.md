# CB Bretagne — Journal de développement (archive)

> **Journal chronologique détaillé, session par session** (ordre anti-chronologique : le plus
> récent en haut). Archive de référence — pour le **contexte courant** (état actuel,
> architecture, dette technique, infra), voir [`NOTES_DEV.md`](NOTES_DEV.md).
>
> Contenu : les résumés « État du projet » des sessions **66 → 8**, puis les logs détaillés
> des sessions **32 → 8**. Les résumés des sessions **67+** vivent dans `NOTES_DEV.md`.

---

## Résumés « État du projet » (sessions 67 → 8)

**État du projet (24/06/2026 — session 67) : Acomptes (double comptage + garde-fou), nouveau format de numérotation, aperçus sans TVA + CP maître d'ouvrage.** Lot de corrections « petits trucs ». **(1) Double comptage de l'acompte** : un acompte est une **avance** déduite du « solde à régler » de la facture de solde (qui porte le montant **plein** du devis), pas une facturation en plus. `Devis.total_facture()` (models.py), son jumeau en mémoire `total_facture_devis()` (totaux.py) et le €/jour réalisé du dashboard (`dashboard_widgets._prod_data`) **excluent désormais** les acomptes (avant : `('facture','acompte','avoir')` → ex. DE04026 affichait Facturé 3070 = 2670 + 400, reste -400). **(2) Garde-fou validation** : à la validation d'une facture de **solde** (1ʳᵉ facture non-acompte du devis) dont un acompte **émis mais pas encore `paid`** existe (`status in ('validated','sent')`), la validation est **bloquée** (la facture reste brouillon) → message « Cette facture mentionne l'acompte FAC-… (X €) qui n'est pas encore marqué « payé ». Confirmez son versement… ». Appliqué à `facture_valider` ET `facture_bypass` (helpers `_acomptes_non_payes_bloquants` / `_msg_acomptes_bloquants`). **Pas d'e-mail** (Brevo pas encore actif). Lever le blocage = marquer l'acompte payé (icône ✓ sur la ligne « Envoyé »). **(3) Nouveau format de numérotation** (arbitrage tranché) — remplace `PRÉFIXE-ANNÉE-NNN` par **préfixe collé + 5 chiffres, SANS année, compteur CONTINU** : devis `DE#####` (compteur propre), facture chantier `FA#####` / acompte `AC#####` / structure `FACTURE-ST#####` (**compteur "FA" partagé**, préfixes d'affichage distincts), avoir `AV#####` (propre) ; appel de convention **inchangé** (legacy `APP-AAAA-NNN`, sera purgé/recalé à la bascule). Code dans `views.py` : `_prochain_numero_nouveau` (scan `^{préfixe}\d+$`, max+1, jamais sous le `floor`), `gen_numero_devis`, `gen_numero_facture` (branche legacy pour appel), constantes `NUM_WIDTH=5`, `NUMEROTATION_FACTURE`, `SEQUENCE_FLOOR`, `DEVIS_FLOOR`, `DEVIS_PREFIX`. **Seeds de TEST** (jeu volontaire au-dessus des réf EBP importées ~02xxx–04xxx pour éviter toute collision pendant les imports) : `DEVIS_FLOOR=7022`, `SEQUENCE_FLOOR={'FA':7054,'AV':1}` ; le seed = **premier numéro attribué**. **À RECALER à la bascule définitive** (devis 4022, FA 2965) en éditant ces constantes. **(4) Devis importés** : conservent le n° du PDF — nouveau champ `Devis.importe_pdf` (BooleanField, migration `0036`), posé `True` dans `import_pdf.create_from_parsed` ; le scan max+1 les ignore tant que leur n° < seed. **(5) Commande `renumeroter_devis`** (`core/management/commands/`) : bascule définitive — renumérote les devis **de l'outil** (`importe_pdf=False`) par n° actuel à partir de `--start` (défaut `DEVIS_FLOOR`), **supprime leurs factures** (de test), **exclut les imports** (saute les numéros réservés pour ne jamais collisionner). **DRY-RUN par défaut**, `--confirm` pour appliquer ; deux phases en transaction (réf temporaires `__RENUM_TMP__<pk>` puis finales) pour respecter l'unicité. **(6) Aperçus sans TVA** : les **devis** ne doivent pas mentionner la TVA — `devis_pdf.html` : ligne « Association non soumise à la TVA — Art. 261-7-1° » supprimée, total « Total HT prestations » → « **Montant net de taxes** », colonne « Total HT » → « **Total** ». Même traitement sur `facture_apercu.html` : ligne TVA + « Total TTC » supprimés (le total reste « Montant net de taxes » / « Solde à régler » si acomptes), colonnes « P.U. HT »/« Total HT » → « P.U. »/« Total ». La mention « TVA non applicable — art. 293B » reste dans le bloc **Conditions générales** (champ `conditions_facture`, géré côté utilisateur). Les **grilles d'édition internes** (éditeurs) gardent « Total HT »/« P.U. HT » — non touchées (décision utilisateur, plus tard). **(7) CP maître d'ouvrage sur l'aperçu devis** : `devis_pdf.html` n'affichait que `client.adresse` (rue) → ajout de la ligne `client.code_postal client.ville` (déjà présente sur la facture). **+7 tests → 225 tests OK + `check` vert.** Vérif **visuelle** navigateur recommandée (aperçus devis & facture : bloc totaux resserré, adresse maître d'ouvrage complète).

**État du projet (24/06/2026 — session 66) : Import devis PDF — OCR de la colonne « Descriptif » + barre de progression.** L'outil d'import de devis PDF (session 62) ne récupérait **aucune description de ligne** : chaque ligne arrivait avec son numéro (`1.1`) en libellé. **Cause racine** (confirmée par inspection pdfplumber des PDF de `docs/exempledevis/`) : EBP exporte la colonne « Descriptif » en **images raster** — une bitmap par cellule (1-bit DeviceGray, ~300 DPI). La couche texte ne contient QUE les nombres (`1.1 | 8,00 | 20,00 | 70,00 | 720,00`). Le parseur travaillant depuis `extract_text()`, il ne pouvait pas voir les libellés. **Solution : OCR (Tesseract `fra`)** + parseur **position-aware**. **(1) `core/import_pdf.py`** : `_number_row_positions(page)` (top de chaque ligne numérotée via `extract_words`, token gauche purement numérique) ; `_descriptif_image_boxes(page)` (images de la colonne descriptif, x0<110 & x1<320, hors logo DeviceRGB) ; `_assign_boxes_to_rows(rows, boxes, total_tops, tol=12)` (**fonction pure**, 3 cas : (a) image alignée ≤ tol à une ligne numérotée = titre/ligne ; (b) image alignée à une ligne « TOTAL » (`_total_row_tops`) = récapitulatif, ignorée ; (c) image orpheline restante = **corps sans numéro** (ex. texte NOTA) rattachée à la ligne numérotée au-dessus → renvoie `{num: [images triées haut→bas]}`, concaténées par l'OCR) ; `_extract_descriptions(pdf)` (rend **chaque page une seule fois** puis recadre en mémoire avec Pillow — bien plus rapide que `page.crop().to_image()` par cellule — et OCR via `pytesseract image_to_string lang='fra' --psm 6`) ; `_apply_descriptions(tree, …)` pose `node['label']`. **Dégradation gracieuse** : si Tesseract indisponible, l'import continue sans descriptions et remonte un **warning** (nouveau champ `parsed['warnings']`, distinct de `errors` — important car la commande CLI **skippe** sur `errors` ; un warning ne doit pas bloquer). **(2) Barre de progression** (l'OCR est lent : ~1-12 s/devis selon le nb de pages) : le JS de `import_devis.html` parse **un PDF par requête** via le nouvel endpoint `import-devis-parse-one` (`views.import_devis_parse_one`, route `core:import-devis-parse-one`), avance la barre au fil des fichiers, puis poste les devis déjà parsés (`results_json`) → `import_devis_view` affiche la preview **sans re-parser**. Le **repli sans-JS** (un seul POST `pdfs`, parse synchrone) est conservé. **Cap `MAX_IMPORT_PDFS=5`** pour rester sous le timeout gunicorn/nginx ; au-delà, message **user-facing** : « créer un dossier sur le SharePoint, y mettre les devis, envoyer le lien à Denis Pekkip » (la commande CLI `import_devis_pdf` reste la voie massive, documentée pour l'admin). Badge preview « OK · sans descriptions » (ambre) si OCR indisponible. **(3) Prérequis Tesseract** : VPS = `apt install tesseract-ocr tesseract-ocr-fra` (sur le PATH, modèle *fast* → ~2× plus rapide que le modèle standard, aucune var d'env) — documenté dans `DEPLOY_OVH.md` §2 + note timeout §7. **Dev Windows** = installer Tesseract puis renseigner `TESSERACT_CMD` (chemin de `tesseract.exe`) et, si `fra` hors du tessdata par défaut, `TESSDATA_DIR` (dossier contenant `fra.traineddata`) — via `.env`. `requirements.txt` +`pytesseract`. **(4) Montants négatifs (financements)** : les lignes de financement EBP sont **négatives** (`2.1 0,90 U -1 000,00 -900,00`) et le parseur perdait le signe → le financement **s'ajoutait** au lieu de se soustraire. Corrigé en 3 points dans `import_pdf.py` : `_NUMBER_RE` capture un `-` optionnel (`r'-?\d[\d ]*,\d{2}'`), `_parse_amount` conserve le signe, et `_create_node` crée les FMAT/FMO si le coût est **≠ 0** (et non plus `> 0`, sinon les négatifs étaient ignorés). **(5) Contrôle total → objet** : `create_from_parsed` compare **systématiquement** `total_brut()` au « Montant Net de Taxes » du PDF ; en cas de **toute** différence au centime (seuil = 0, les sommes doivent être identiques) **ou** de Montant introuvable, marque l'**objet du devis** (champ `chantier`, visible en liste) d'un préfixe `⚠ ÉCART TOTAL X € — …` + note détaillée (avant : note seule). **(6) Recoupement par ligne** : chaque feuille imprime son « Total Net de Taxes » (dernière colonne) — capté dans `total_str` (`_parse_amounts_from_text` renvoie désormais ce 5ᵉ champ). `_create_node` compare, au centime, le total calculé (`qté × (mat+mo)`) à ce total EBP ; en cas d'écart, **préfixe le descriptif de la ligne** d'un marqueur `⚠ ÉCART Z € — Total calculé = X € / Montant EBP = Y €` → on repère **quelle(s) ligne(s)** portent l'écart global (ex. DE03137 : 4 lignes cumulant le 0,31 €, arrondis EBP sur PU non affichés). **Surcoût nul** (arithmétique à la création, ~0,08 s ; le temps d'import reste celui de l'OCR ~12-15 s). **+13 tests (`ImportDevisOcrTests`, dont 3 `@skipUnless` Tesseract+PDF présents) → 218 tests OK + `check` vert.** Vérifié sur les 6 PDF de `docs/exempledevis/` : ~100 % des lignes reçoivent leur libellé (DE03137 8 pages/81 nœuds = 11,7 s en *fast*) ; **totaux recoupés** (DE04026 2670 ✓, DE04079 16514,94 ✓, FAURE2 *avec aides* 420,70 ✓ — financements négatifs OK, DE03065 655 ✓ et *sans financements* 2903 ✓ ; DE03137 75759,56 vs 75759,25 → flag 0,31 € arrondi EBP, attendu). ⚠️ Les PDF d'exemple (`docs/exempledevis/`, vrais devis clients) **ne sont pas commités** (confidentialité) ; les tests bout-en-bout se *skippent* s'ils sont absents. **Nettoyage Railway** (suite migration OVH) : suppression des fichiers de déploiement morts `railway.toml` / `Procfile` / `runtime.txt` (OVH = gunicorn systemd + `deploy.sh`) ; valeurs par défaut `CSRF_TRUSTED_ORIGINS` et `SITE_URL` de `settings.py` repassées sur l'URL OVH (ne plus pointer vers l'ancienne URL Railway) ; commentaires `settings.py`/`deploy.sh` + `docs/plan_ameliorations.md` (« push = déploiement ») corrigés. Les mentions Railway restantes dans NOTES_DEV sont **historiques** (journal).

**État du projet (23/06/2026 — session 65) : Outillage démo équipiers + champ « Entrée CBB ».** Petits travaux post-refonte planning (aide au test sur le VPS). **(1) Commande `seed_equipiers`** (`core/management/commands/seed_equipiers.py`) : génère des **équipiers fictifs** (et rien d'autre — ne crée aucune équipe) rattachés aux **équipes permanentes du service Insertion 35** (`service.module_planning=True`, `type_rangee='permanente'`). Effectif par défaut = `equipe.nb_equipiers` (override `--per-equipe N`) ; options `--equipe <texte>` (filtre nom icontains), `--all-equipes`, `--clear`. **Marqueur** : matricule `FIC-<id_équipe>-<NN>` → idempotent (`get_or_create`) et `--clear` ne supprime QUE ces équipiers (+ présences/prêts par cascade `CASCADE`). Données réalistes : contrat uniforme `CDDI - 26 heures` mais **28h/semaine effectives** (`heures_contrat_hebdo=28`, cas réel des CDDI : récup = 2h/sem), 3 dates remplies (`date_entree_cbb` ≤ `date_debut_contrat` ≤ `date_fin_contrat`), `random.Random(42)` déterministe. **(2) Champ `Equipier.date_entree_cbb`** (DateField null/blank, migration `0035`) — date d'entrée dans la structure, **distincte** du début de contrat (un équipier peut enchaîner plusieurs contrats). `date_debut_contrat`/`date_fin_contrat` existaient déjà. Câblé dans le **formulaire équipier** (`equipiers.html` + `equipier_save`) : champ « Entrée CBB » dans la modale ajout/édition (pré-rempli via `data-entree`), pour correction manuelle après l'import futur de la vraie base RH. Visible aussi dans l'admin Django. **205 tests + `check` verts.** NB : ces données démo seront remplacées par l'import du fichier des contrats de travail (pas de règle métier figée ici).

**État du projet (23/06/2026 — session 64) : Planning Phase 4b — calendrier de plage commun aux 3 modales.** Suit `REFONTE_PLANNING.md` Phase 4b (handoff §5E + maquette `Modales harmonisées.dc.html`) — **clôt la Phase 4 et la refonte planning**. **(1) Composant partagé `RangeCalendar(opts)`** dans `core/static/core/app.js` : rend une grille de mois (7 col.), gère la sélection d'une plage + navigation de mois. Deux modes — `'range'` (clic-glissé début→fin) et `'start'` (clic = début, fin via `opts.computeEnd`). Options : `grid`, `monthLabel`, `isBusy(iso)→false|true|titre` (marque `.busy`), `onChange(s,e)`. API : `nav/goToMonth/setRange/clear/getStart/getEnd/render`. **Ne rend RIEN à la construction** (sûr si la grille est absente, ex. émargement sans `peut_modifier`). **(2) Styles `.cal*` communs** dans `app.css` (plage **turquoise** `.s/.e/.in`, jours pris **ambre** `.busy`, aujourd'hui **point prune** `.today::after`) — conformes maquette. Suppression des CSS calendrier dupliqués des 2 templates (`.cal-day/.cal-hdr/.pret-cal*/.cal-busy/.ema-week`). **(3) Rebranchement des 3 modales** : *affectation* (`planning.html`, mode `start` — clic = début, fin calculée en jours ouvrés via `addWorkingDays`, jours déjà affectés à l'équipe en ambre via `AFF_PAR_EQUIPE`) ; *prêt* (`emargement.html`, mode `range` + 2 créneaux Matin/Après-midi conservés) ; *événement* (`planning.html`, mode `range`) — **les 2 `<input type="date">` natifs (`ev-debut`/`ev-fin`) sont supprimés** et remplacés par le calendrier ; `submitEv` envoie `date_fin` = fin seulement si ≠ début (sinon `null`, jour unique). Anciennes fns `renderCal/renderAffCal/calNav/affCalNav` + état `_calAnchor/_calHover/_affCalYear…` supprimées. **Modèle/endpoints inchangés** (payloads identiques : `presence_reassign` n/a ; `pret_save`, `evenement_save`, `affectation_save` reçoivent les mêmes champs). La modale **rangée ponctuelle** (Phase 3) garde ses dates natives — hors périmètre §5E. **205 tests + `check` verts** (pas de test JS ; backend inchangé). ⚠️ **Vérif visuelle navigateur obligatoire** : ouvrir les 3 modales, tester sélection/glissé/nav mois, créer un prêt + un événement + une affectation. Manuel `/aide/` : pas de MAJ (même grammaire d'usage, ergonomie calendrier améliorée).

**État du projet (23/06/2026 — session 63) : Planning Phase 4a — logos financeurs sur la feuille de présence.** Suit `REFONTE_PLANNING.md` Phase 4 (volet **4a** uniquement ; 4b — calendrier de plage commun aux 3 modales — reste à faire). **(1) Modèle** : nouveau champ `Financeur.logo = ImageField(upload_to='financeurs/', blank=True, null=True)` (migration `0034`) — Pillow déjà présent, pattern d'upload déjà utilisé par `ParametresAssociation.logo`. **(2) Admin** : `FinanceurAdmin` expose `logo` (list_display + `fields`) → logos téléversables depuis l'admin (PNG/SVG). **(3) Feuille** (`presence_feuille.html`) : le pied de page itère sur `equipe.financeurs.all` et affiche `<img class="logo-img">` si `fin.logo`, sinon le **nom dans un cadre** `.logo-nom` (fallback, comportement antérieur). CSS calé sur la maquette `Feuille de présence.dc.html` : `.logos` aligné à droite, **hauteur fixe 30 px** (`.logo-img`/`.logo-nom`) pour tenir sur **1 page A4 paysage**. **(4) Prod** : `/media/` est déjà servi par nginx (`location /media/ { alias … }`, volume persistant, cf. `DEPLOY_OVH.md`) ; en dev, `urls.py` sert media via `static()` quand `DEBUG=True`. Aucune nouvelle infra. **+2 tests** (`test_feuille_logo_image` avec PNG 1×1 base64 + `MEDIA_ROOT` temporaire, `test_feuille_logo_fallback_nom`). **205 tests OK + `check` vert.** ⚠️ Vérif **visuelle** navigateur (impression A4) recommandée. Manuel `/aide/` : pas de MAJ nécessaire (changement purement visuel, upload côté admin).

**État du projet (23/06/2026 — session 62) : Import PDF de devis EBP.** Nouvel outil d'import des devis historiques au format PDF (généré par EBP / ancien logiciel CBB Bretagne). **Fichiers créés/modifiés :** `core/import_pdf.py` (parseur complet), `core/views.py` (+2 vues `import_devis_view` + `import_devis_confirm`), `core/urls.py` (+2 routes), `core/templates/core/import_devis.html` (UI 2 étapes upload → preview → confirm), `core/templates/core/base.html` (lien sidebar), `core/management/commands/import_devis_pdf.py` (commande CLI batch), `requirements.txt` (+pdfplumber). **Parsing (`import_pdf.py`) :** extraction via `page.extract_text()` (pas `extract_table()` — trop instable). Métadonnées : référence, date, date_validité, secteur/équipe_hint, objet, bloc adresse chantier + client (regex labeled). Structure : `_collect_text_lines()` → `_parse_text_row()` → `_build_tree()` ; chaque nœud feuille → S + FMAT + FMO ; nœuds intermédiaires → TITRE. Corrections clés de la session : (1) regex N° `^(\d+(?:\.\d+)*)(?:\s+(.*))?$` (whitespace obligatoire après N° — empêche "0,00" d'être parsé comme N°=0 ce qui causait de faux nœuds racine) ; (2) détection de ligne de continuation pour les montants splittés sur 2 lignes par pdfplumber ("4.1 81,00 M2" + "26,18 2 120,58" → fusionnés) ; (3) correction des guillemets typographiques U+2019 introduits par l'outil d'édition (remplacés par apostrophes ASCII). **Checksum :** compare `devis.total_brut()` vs "Montant Net de Taxes" du PDF — seuil 0,01 € ; note dans `devis.notes` si dépassé avec message « Différence de X €, probable erreur d'arrondi venant d'EBP. À vérifier si cela semble anormal ». **Résultats sur 4 PDFs de test :** DE04124 (12 lignes, 0 € écart ✓), DE03137/Alet (149 lignes, 0,31 € arrondis EBP), DE02649/Makhowski (24 lignes, 0 € écart ✓), DE03223/Aurin (184 lignes, 0,03 € arrondis EBP). **Limitation connue :** la colonne "Descriptif" (libellés des lignes) est absente de la couche texte du PDF — EBP l'exporte probablement en graphique rasterisé. Les lignes sont importées sans description (uniquement le numéro). À traiter ultérieurement (OCR ou saisie manuelle). **Tests :** flux web validé via test client Django (upload → preview → confirm) sur les 4 PDFs.

**État du projet (22/06/2026 — session 61) :** **Planning Phase 3 — Rangées ponctuelles (temporaire / renfort / prestataire).** Suit `REFONTE_PLANNING.md` Phase 3 (handoff §0 décision 5 + §3 + §6). **(1) Modèle** : 4 champs sur `Equipe` (migration `0033`) — `type_rangee` (`permanente`|`temporaire`|`renfort`|`prestataire`, défaut `permanente`), `date_fin_temp` (DateField, temporaire), `mo_forfait` (Decimal €, renfort), `archivee` (Bool) ; propriétés `est_ponctuelle` / `sans_emargement` ; champs ajoutés à `EquipeAdmin`. **Une rangée ponctuelle = une `Equipe` ad-hoc portant UN chantier (une `Affectation`)** — le moins invasif, tout le rendu itère déjà sur les équipes. **(2) Création sans admin Django** : endpoint `rangee_save` (`views_planning.py`, route `core:rangee-save`, garde `peut_modifier_insertion`) crée l'`Equipe` (service insertion de l'utilisateur, sinon 1ᵉʳ service `module_planning`) **puis** une `Affectation` sur la tranche choisie ; pour temporaire, crée les `Pret(equipe_hote=…)` depuis les équipiers sélectionnés. **NE recalcule PAS** `_recalcul_durees_tranche` (dates explicites saisies, ne décale pas les permanentes du même devis). Toolbar planning : bouton « Rangée ponctuelle » → modale `#modal-rangee` (sélecteur 3 cartes radio + nom + devis/tranche via `planning_wizard_data` + dates natives + équipiers à prêter / montant forfait selon type ; JS `openRangeeModal`/`rangeeSetType`/`rangeeFillTranches`/`submitRangee`, `EQUIPIERS_DISPO` injecté). **(3) Rendu** (`planning.html` + CSS) : dict `ligne` enrichi (`type_rangee`, `mo_forfait`, `nb_prets`) ; label par type (avatar pointillé prune `.av-temp` + badge « Temp. » + « N équipiers prêtés · jusqu'au jj/mm » ; avatar ambre + badge « MO forfait » ; avatar gris outil `.av-prest` + « Prest. ») ; barre renfort **ambre avec montant €** au lieu du %, barre prestataire **hachurée grise `.tl-bar.prest`** (toujours lecture seule — `peut_modifier` forcé False côté vue). **(4) Émargement** : sélecteur d'équipes exclut `renfort`/`prestataire` + `archivee=True` ; la temporaire (vide) affiche ses équipiers via les prêts existants (`grid_rows_empruntes`). **(5) Archivage auto** : passe paresseuse en tête de `planning_mois` (`type_rangee='temporaire', date_fin_temp__lt=today → archivee=True`), exclue de la grille. **(6) Rentabilité** : renfort/prestataire ne créent **aucune** `Presence` → `pct_consomme`/`heures_par_tranche` des permanentes **identiques** ; `mo_forfait` **stocké + affiché** seulement, agrégation au MO consommé du dashboard insertion **différée** (TODO posé dans `dashboard_widgets._prod_data`). **+6 tests `RangeesPonctuellesTests` · 203 tests OK.** ⚠️ Vérif **visuelle** navigateur de la modale + des 3 rendus de barre à faire.

**État du projet (22/06/2026 — session 60) :** **Planning Phase 2 — Émargement : permissions & codes d'événements.** Suit `REFONTE_PLANNING.md` Phase 2 (handoff §2 + §3 `code_absence`). **(1) Permission élargie (`can_modifier_insertion`)** : nouvelle fonction `peut_modifier_insertion(user)` dans `permissions.py` — admin/responsable **ou** membre d'un service `module_planning=True` **ou** encadrant d'au moins une équipe active du module. `peut_acceder_planning` élargi dans le même sens (accès page). `emargement_view` : queryset équipes + `peut_modifier` → `peut_modifier_insertion`. `planning_mois` : `peut_modifier_global` + `equipes_modifiables_ids` → `peut_modifier_insertion` (tout membre insertion peut éditer toutes les lignes planning). `presence_save` + `presence_reassign` : gate `est_encadrant(…)` per-équipe → `peut_modifier_insertion` global. **(2) `Evenement.code_absence`** : nouveau champ `CharField(max_length=10, blank=True, choices=…)` sur `Evenement` (migration `0032`). `evenement_save` lit/écrit ce champ ; `evenements_data_json` l'inclut. Modale `.modal-ev` (`planning.html`) : select « Code d'absence attribué » + reset/chargement JS (`openModalEv`, `submitEv`). **(3) Propagation dans l'émargement** : `_build_row_cells` — `special_code` étendu : `R` (pont) > `ev_code` (1ᵉʳ événement non-ferié avec `code_absence` couvrant le jour) > `F` légal. **(4) Propagation dans la feuille de présence** : `presence_feuille` — `special_map` enrichi par les événements `code_absence` (priorité : F légal < événement < R pont). **(5) Tests** : `test_permission_membre_insertion_peut_modifier` (non-encadrant service insertion → saisie présence OK) + `test_evenement_code_absence_propagation` (event code=F → special_code=F en émargement). **197 tests OK.** Feuille de présence : `est_encadrant` conservé (encadrant + RH, inchangé).
...
passe fonctionnels. Page `/manuel/` publique (manuel utilisateur HTML). Correctifs sécurité
...
### Livré
- **Page manuel Insertion** (`/insertion/aide/`) — page autonome (hors layout app) avec table des matières latérale, même style CSS que `manuel.html` (Montserrat, prune, teal)
...
- `core/views.py` :
  - `manuel_view` — page publique (pas de `@login_required`)
...
- `core/urls.py` — routes `/manuel/` et `/mot-de-passe-oublie/`
- `core/templates/core/manuel.html` — page manuel utilisateur autonome (pas de base.html) :
  TOC sticky, 12 chapitres, schémas inline, 11 captures PNG (`core/static/core/manuel/`),
  lien "← Retour à l'application" encadré vert — mis à jour session 71 (27/06/2026)
...
- **Manuel `/manuel/`** : page HTML publique (sans `@login_required`), maintenu manuellement
  à chaque grosse session. Lien dans la sidebar + dans l'email d'invitation.
  Captures dans `core/static/core/manuel/` (PNG, servis via WhiteNoise). Manuel insertion
  séparé : `/insertion/aide/` → `aide_insertion.html` (non touché).
